      ...
      if data ~= nil then
        print(topic .. ": " .. data)
          ...
          if(topic == config.ENDPOINT .. config.ID .. "/commands/powerSW1") then
              if(data == "ON") then gpio.write(config.SSW1,gpio.LOW)
              elseif(data == "OFF") then gpio.write(config.SSW1,gpio.HIGH)
              end
          ...
          elseif(topic == config.ENDPOINT .. config.ID .. "/commands/powerSW10A1") then
              if(data == "ON") then gpio.write(config.SW110A1,gpio.HIGH)
              elseif(data == "OFF") then gpio.write(config.SW110A1,gpio.LOW)
              end
          elseif(topic == config.ENDPOINT .. config.ID .. "/commands/powerSW10A2") then
              if(data == "ON") then gpio.write(config.SW110A2,gpio.HIGH)
              elseif(data == "OFF") then gpio.write(config.SW10A2,gpio.LOW)
              end
