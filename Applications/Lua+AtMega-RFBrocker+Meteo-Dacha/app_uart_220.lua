...
local function split(str, pat)
  ...
  local function publish_motion()
    ...
    local motion = gpio.read(config.SCL)
    if (motion ~= motionState) then
      motionState = motion
          m:publish(config.ENDPOINT .. config.ID .. "/move",config.STATUS[motion],0,0, function()
            m:publish(config.ENDPOINT .. config.ID .. "/lwt","ON",0,0, function()
              print("Publish has been completed")
              checkMQTT = 1
              MQTTfailCount = 0
...
local function mqtt_connect()
  m:connect(mqttConfig.HOST, mqttConfig.PORT, config.MQTT_SSH, 0, function(con)
...
--      tmr.alarm(tmrMQTTPublish, config.publishTime, 1, publish_switches)
      ...
      tmr.alarm(tmrMotion,1000,tmr.ALARM_AUTO,publish_motion)
...
--  tmr.interval(tmrLED, 600)
  ...
  gpio.mode(config.SCL, gpio.INPUT)
  mqtt_start()
end
